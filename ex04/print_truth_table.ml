...
let print_truth_table (formula: string): Unit.t = 
  ...
  let rec print_truth_rec i =
    ...
    then (
      let bin = Utils.Int_to_bin_str.int_to_bin_str (Int32.of_int i) (String.length arg) in print_truth_line (bin ^ String.make 1 (if (Common.Eval_formula.eval_formula (Utils.Calc.calc bin arg formula)) = true then '1' else '0'));
      print_truth_rec (i + 1)
    )
