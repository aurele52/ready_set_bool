...
let conjunctive_normal_form (str: string): string = btree_to_RPN_app (distrib (btree_construct (negation_normal_form str)))


let test = "AB=CD=|"      (* (A=B) | (C=D) *)
let () = print_tr (test)
let () = print_truth(conjunctive_normal_form test)
